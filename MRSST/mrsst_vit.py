...
# 获取相邻波段像素
def gain_neighbor_band(x_train, band, neighbor_band, patch=9):
    # (64,13,13,200)
    ...
    x_train_band = x_train_band.reshape(x_train_band.shape[0], band, x_train_band.shape[1])  # 64,200,7
...
class MRTokenG(nn.Module):
    ...
    def __init__(self, dim,patch, neighbor_band, num_patches):
        super().__init__()
    ...
    def forward(self, x_raw,x_fe):
        ...
        x_spe = self.conv1d_1(x_spe)
        x_fe = x_fe.flatten(2)
        x_fe = self.fc_layers(x_fe)
        x = torch.cat((x_fe, x_spe),2)
...
class mrsst(nn.Module):
    ...
    def __init__(self, patch, neighbor_band, num_patches, num_classes, dim, depth, heads, mlp_dim, pool='cls',
                 channels=1, dim_head=16, dropout=0., emb_dropout=0., mode='CAFM'):
        super().__init__()
        self.cfpe =CFPE_module(neighbor_band=neighbor_band,num_patches=num_patches)
        self.TG = MRTokenG(dim =dim,patch=patch,neighbor_band=neighbor_band,num_patches=num_patches)
        self.L = num_patches
